...
### P0 (headline, user-confirmed) — Claude Code cursor invisible: reverse-video on the glass backdrop painted nothing
- **This is the real cause of the reported "caret doesn't render in claude."** Empirically, claude **hides the OS cursor** (`ESC[?25l`, never re-shown) and draws its cursor as a **reverse-video (`ESC[7m`) cell**. SwiftTerm resolves a reverse-video *default-background* cell's fill to `nativeBackgroundColor.inverseColor()`. Termy uses a transparent glass backdrop, so `nativeBackgroundColor` is `NSColor.clear` (alpha 0) — and `inverseColor()` preserves alpha, so the inverted background is **still fully transparent**. The reverse cell painted nothing → no visible cursor (and any default reverse-video text was invisible too).
- **Fix (`TERMY_PATCH inverse_bg_opaque`):** when the backdrop is (near-)transparent, a reversed default background falls back to the opaque foreground colour, so the block is visible (light block + dark glyph — standard reverse-video). Opaque hosts are unaffected.
- **Verified end-to-end:** rebuilt the app, ran `claude`, and confirmed via screenshot that the cursor now renders as a visible block on the input placeholder (it was an invisible gap before). Plus `ReverseVideoColorTests` pins the transparency predicate the patch keys on.

### P0 — Caret in claude / synchronized-output rendering (contributing fixes)
